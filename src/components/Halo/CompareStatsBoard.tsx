...
const CompareStatsBoard = ({ statKeys, stats, loading }: CompareStatsBoardProps) => {
  ...
  return (
    <>
      {betterStats && (
        <>
          ...
          >
            ...
            >
              <Flex sx={{ fontSize: [2, 3], flex: 1, textTransform: "uppercase", letterSpacing: 1 }}>Evans</Flex>
            ...
            >
              <Flex sx={{ fontSize: [2, 3], flex: 1, textTransform: "uppercase", letterSpacing: 1 }}>{stats.tag.name}</Flex>
              <Flex sx={{ fontSize: 4, fontWeight: "bold" }}>
      ...
      {hasFetchedOn && (
        <Box sx={{ fontSize: "10px", paddingY: 2, opacity: 0.8 }}>
          {stats?.me?.fetchedOn && <Box>My stats as of {dayjs(stats?.me?.fetchedOn).format("LLLL")}</Box>}
          {stats?.tag.data?.fetchedOn && (
            <Box>
              {stats?.tag?.name} stats as of {dayjs(stats?.tag?.data?.fetchedOn).format("LLLL")}
            </Box>
          )}
          <Box>To minimize impact on various services and tools, data is cached for atleast one hour.</Box>
        </Box>
      )}
    </>
  );
